...
void ConsoleAppender::installCustomLogHandler(CuteLoggerMessageHandler pFunc) 
{
  m_handlerPFN = pFunc;
}
...
{
  bool pass = true;
  if (m_handlerPFN && !m_handlerPFN(logLevel, message)) {
    pass = false;
  }
  if (pass) {
    std::cerr << qPrintable(formattedString(
